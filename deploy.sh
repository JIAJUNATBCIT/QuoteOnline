...
build_frontend() {
  ...
  if node -e "const p=require('./package.json');process.exit(p.scripts&&p.scripts['build:optimized']?0:1)" >/dev/null 2>&1; then
    ...
    ng build --configuration production --no-interactive --parallel "$DOCKER_BUILD_PARALLEL" >/dev/null 2>&1
  fi
  ...
  if docker network inspect "$network_name" >/dev/null 2>&1; then
    ...
    while [[ $retry -gt 0 ]]; do
      if docker network rm "$network_name" >/dev/null 2>&1; then
        ...
        retry=$((retry-1))
        warn "删除网络失败，剩余重试次数：$retry，等待 ${NETWORK_WAIT}秒..."
        sleep $NETWORK_WAIT
      fi
    done
    warn "网络 $network_name 删除失败，忽略并继续部署（Docker会自动处理）"
  fi
}
...
compose_up_http() {
  ...
  while [[ $retry -gt 0 ]]; do
    if [[ ! -f "$build_flag" ]]; then
      if docker compose down >/dev/null 2>&1 || true && docker compose up -d --build >/dev/null 2>&1; then
        touch "$build_flag"
        ok "容器启动完成"
        return
      fi
    else
      if docker compose down >/dev/null 2>&1 || true && docker compose up -d >/dev/null 2>&1; then
        ok "容器启动完成"
        return
      fi
    fi
    ...
    retry=$((retry-1))
    warn "容器启动失败（网络错误），剩余重试次数：$retry，等待 ${NETWORK_WAIT}秒..."
    sleep $NETWORK_WAIT
    cleanup_docker_network "$network_name"
  done
